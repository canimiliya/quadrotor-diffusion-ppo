"""Train and audit S4-R2 with frozen S2-TRAIN observation standardization."""
...
from quadrotor_diffusion_ppo.ppo.normalization import (
    FixedObservationStandardizer, derive_train_statistics, sha256_file,
)
...
R1_SUMMARY = ROOT / "artifacts" / "s4r1" / "summary.json"
TRAIN_NPZ = DATASET / "dataset" / "train.npz"
...
def dataset_hashes(*, include_test: bool = False) -> dict[str, str]:
    values = {}
    names = ["train.npz", "val.npz"] + (["test.npz"] if include_test else [])
    for name in names:
...
def load_r1_summary() -> dict[str, Any]:
    """Read the immutable R1 result for the required side-by-side audit."""
    return json.loads(R1_SUMMARY.read_text(encoding="utf-8"))


def policy_kwargs(statistics) -> dict[str, Any]:
    return {
        "net_arch": {"pi": [256, 256], "vf": [256, 256]},
        "features_extractor_class": FixedObservationStandardizer,
        "features_extractor_kwargs": {
            "mean": statistics.mean.astype(float).tolist(),
            "scale": statistics.scale.astype(float).tolist(),
        },
...
def normalized_sensitivity(model: PPO, observations: np.ndarray) -> dict[str, Any]:
    """Repeat D0's dimensionless input sensitivity audit on frozen TRAIN samples."""
    rng = np.random.default_rng(SEED)
    count = min(4096, len(observations))
    sample = np.asarray(observations[rng.choice(len(observations), count, replace=False)], dtype=np.float32)
    tensor = torch.as_tensor(sample, dtype=torch.float32, device=model.device).requires_grad_(True)
    actions = model.policy.get_distribution(tensor).mode()
    per_feature = []
    for action_index in range(3):
        gradient = torch.autograd.grad(actions[:, action_index].sum(), tensor, retain_graph=True)[0]
        per_feature.append(gradient.detach().abs().cpu().numpy())
    jacobian_abs = np.stack(per_feature, axis=0).mean(axis=0)
    raw_std = np.std(observations.astype(np.float64), axis=0)
    dimensionless = jacobian_abs * raw_std[None, :]
    goal = float(dimensionless[:, 3:6].mean())
    rays = float(dimensionless[:, 16:34].mean())
    return {"sample_count": count, "goal_delta_mean": goal, "rays_mean": rays,
            "ray_to_goal_ratio": rays / max(1.0e-12, goal)}
...
def main() -> None:
    ...
    probe.close()
    raw_probe = PurePPONavigationEnv(
        scenes[tasks["train"][0].scene_id], tasks["train"][0], train_mode=False, gui=False
    )
    probe_observation, _ = raw_probe.reset(seed=tasks["train"][0].candidate_seed)
    raw_probe.close()
    normalized_probe = (probe_observation.astype(np.float64) - statistics.mean) / statistics.scale
    if normalized_probe.shape != (34,) or not np.isfinite(normalized_probe).all():
        raise RuntimeError("S4-R2 normalization preflight failed")
    normalization_contract = statistics.contract() | {"normalization_sha256": statistics.contract_sha256}
    (ARTIFACTS / "normalization.json").write_text(
        json.dumps(normalization_contract, indent=2), encoding="utf-8"
    )
    ...
    try:
        model = PPO(UnitBallActorCriticPolicy, train_env, learning_rate=PPO_CONFIG["learning_rate"],
                    ...
                    policy_kwargs=policy_kwargs(statistics),
    ...
    best_model_for_audit = PPO.load(str(best_path), device="cuda")
    loaded_mean = best_model_for_audit.policy.features_extractor.fixed_mean.detach().cpu().numpy()
    loaded_scale = best_model_for_audit.policy.features_extractor.fixed_scale.detach().cpu().numpy()
    if not np.array_equal(loaded_mean, statistics.mean.astype(np.float32)) or not np.array_equal(
        loaded_scale, statistics.scale.astype(np.float32)
    ):
        raise RuntimeError("checkpoint did not preserve frozen normalization")
    val_gate = bool(
    ...
    test_run_count = 0
    if val_gate:
        test_rows, test_summary = evaluate(best_model_for_audit, tasks["test"], scenes, actual_steps)
        test_run_count = 1
    ...
    r1 = load_r1_summary()
    r1_best = r1["best_val"]
    r1_family = r1["val_by_family"]
    with np.load(TRAIN_NPZ, allow_pickle=False) as train_data:
        sensitivity = normalized_sensitivity(best_model_for_audit, train_data["observations"])
    final_label = (
        "PASS_S4R2_FIXED_NORMALIZED_PURE_PPO_BASELINE" if val_gate else
        "BLOCKED_S4R2_NORMALIZATION_IMPLEMENTATION"
    ...
    summary = {
        "task": "S4-R2-TRAIN-ONLY-OBSERVATION-STANDARDIZATION-AND-PURE-PPO-BASELINE-CLOSURE-V1",
        "final_label": final_label,
        "canonical_main": "b10c8edf2fc834aac9136626a969c0c3108820ae",
        "main_fast_forward": "PENDING_PASS_ONLY" if val_gate else "NOT_AUTHORIZED_VAL_GATE_FAIL",
        "branch": "agent/s4r2-observation-standardization-v1",
        "start_head": "8f02f831821b84cb0cb53564892797e18edd4ba3",
        "end_head": None, "remote_branch_head": None,
        "r1_policy_head": "845353c0a83651464279711328a3b168c1a6a065",
        ...
        "ppo_config_hash_r1": PPO_CONFIG_HASH,
        "normalization": normalization_contract,
        "normalization_checkpoint_persistence": "PASS",
        "actor_critic_identical_preprocessing": True,
        "raw_environment_observation_unchanged": True,
        "dataset_sha256": dataset_hashes(include_test=val_gate), "train_tasks": len(tasks["train"]),
        ...
        "r1_to_r2": {
            "delta_success": int(best_summary["success"]) - int(r1_best["success"]),
            "delta_collision": int(best_summary["collision"]) - int(r1_best["collision"]),
            "delta_timeout": int(best_summary["timeout"]) - int(r1_best["timeout"]),
            "delta_mean_return": float(best_summary["mean_return"]) - float(r1_best["mean_return"]),
            "action_clip_r1": float(r1_best["action_clip_fraction"]),
            "action_clip_r2": float(best_summary["action_clip_fraction"]),
            "by_family": {family: {"r1": r1_family[family], "r2": best_summary["by_family"][family]}
                          for family in ("OPEN", "BLOCK", "SBEND")},
        },
        "ray_goal_sensitivity": {"r1": 0.010383593954940604, "r2": sensitivity},
        ...
        "formal_progress": "55%" if val_gate else "45%",
        "test_accessed": bool(val_gate),
        "controller_note": None if val_gate else "A future controller-authorized budget study may consider 5,000k then 20,000k; it was not run in S4-R2.",
    }
    ...
    print(f"S4-R2 training complete; best VAL={best_record['summary']['success']}/54 at {best_record['env_steps']}; TEST={test_text}", flush=True)
...
def smoke_main() -> None:
    ...
    try:
        ...
        diagnostics = env.env_method("episode_diagnostics")
        with tempfile.TemporaryDirectory(prefix="s4r2-smoke-") as temporary_directory:
            checkpoint = Path(temporary_directory) / "smoke_model"
            model.save(str(checkpoint))
            loaded = PPO.load(str(checkpoint.with_suffix(".zip")), device="cuda")
            mean_match = torch.equal(
                model.policy.features_extractor.fixed_mean,
                loaded.policy.features_extractor.fixed_mean,
            )
            scale_match = torch.equal(
                model.policy.features_extractor.fixed_scale,
                loaded.policy.features_extractor.fixed_scale,
            )
        print(json.dumps({"smoke": "PASS", "timesteps": int(model.num_timesteps),
                          "device": str(model.device), "n_envs": env.num_envs,
                          "normalization_checkpoint_roundtrip": bool(mean_match and scale_match),
                          "action_clip_count": int(sum(item["action_clip_count"] for item in diagnostics))}), flush=True)
    finally:
        env.close()
